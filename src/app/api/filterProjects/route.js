...
export async function POST(req){
    ...
    for(var obj in body.data){
        let result = body.data[obj].match(frontendRegex);
        if(result){
            frontend.push(body.data[obj].toLowerCase());
            
        }
        result = body.data[obj].match(backendRegex);
        if(result){
            backend.push(body.data[obj].toLowerCase());
            
        }
        result = body.data[obj].match(databaseRegex);
        if(result){
            database.push(body.data[obj].toLowerCase());
            
        }
        result = body.data[obj].match(infrastructureRegex);
        if(result){
            infrastructure.push(body.data[obj].toLowerCase());
